...
import { FaPhoneAlt, FaWhatsapp, FaShieldAlt } from "react-icons/fa";
import "@/styles/tempo-traveller.css";
...
const VEHICLES = [
  {
    badge: "12 Seater", badgeColor: "blue",
  ...
  {
    badge: "14 Seater", badgeColor: "blue",
  ...
  {
    badge: "16 Seater", badgeColor: "blue",
  ...
  {
    badge: "18 Seater", badgeColor: "orange",
  ...
  {
    badge: "20 Seater", badgeColor: "orange",
  ...
  {
    badge: "24 Seater", badgeColor: "orange",
  ...
  {
    badge: "Maharaja", badgeColor: "orange",
...
const OPTIONS_TABLE = [
  { vehicle: "12 Seater Tempo Traveller in Kanpur", capacity: "12 + Driver", ac: "AC",         ideal: "Small and medium family groups - local sightseeing, day trips, and outstation routes to Lucknow and Prayagraj" },
  { vehicle: "14 Seater Tempo Traveller in Kanpur", capacity: "14 + Driver", ac: "AC",         ideal: "Medium family groups and office teams - ideal for corporate outings, school trips, and comfortable outstation travel" },
  { vehicle: "16 Seater Tempo Traveller in Kanpur", capacity: "16 + Driver", ac: "AC",         ideal: "Medium to large groups - corporate tours, school excursions, wedding parties, and longer routes to Varanasi and Delhi" },
  { vehicle: "18 Seater Tempo Traveller in Kanpur", capacity: "18 + Driver", ac: "AC",         ideal: "Large family gatherings and pilgrimage groups - comfortable seating for trips to Delhi, Agra, and Ayodhya" },
  { vehicle: "20 Seater Tempo Traveller in Kanpur", capacity: "20 + Driver", ac: "AC",         ideal: "Large groups - baraat transport, religious pilgrimage groups, and large family gatherings travelling together" },
  { vehicle: "24 Seater Tempo Traveller in Kanpur", capacity: "24 + Driver", ac: "AC",         ideal: "Very large groups - big corporate events, college excursions, and multi-city pilgrimage tours across North India" },
  { vehicle: "Luxury Tempo Traveller in Kanpur",    capacity: "9–16 + Driver", ac: "Premium AC", ideal: "VIP guests, wedding functions, corporate travel - soft pushback seats, LED lighting, music system, charging points" },
  { vehicle: "Maharaja Tempo Traveller in Kanpur",  capacity: "9–16 + Driver", ac: "Premium AC", ideal: "Ultimate VIP travel, grand weddings, senior citizens - sofa seats, full recline, premium interiors, air suspension" },
];
...
export default function TempoTravellerKanpur() {
  ...
  return (
    <>
      ...
      {/* TOP BAR */}
      <div style={{ background: "#0f6ec8", padding: "9px 0" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <a href="https://wa.me/919044019511" style={{ color: "#fde68a", fontSize: 13, fontWeight: 600 }}>WhatsApp Us</a>
            <a href="tel:+919044019511"          style={{ color: "#fde68a", fontSize: 13, fontWeight: 600 }}>+91 90440 19511</a>
          </div>
        </div>
      </div>
        ...
        <div className="hero-inner">
          ...
          <div className="booking-card" id="booking">
            <div className="booking-tabs">
              {tabs.map(tab => (
                <button key={tab} className={`b-tab${activeTab === tab ? " active" : ""}`} onClick={() => setActiveTab(tab)}>{tab}</button>
              ))}
            </div>
            <div className="form-grid">
              <div>
                <div className="fg-label">From</div>
                <input className="fg-input" readOnly value="Kanpur" />
              </div>
              <div>
                <div className="fg-label">To</div>
                <input className="fg-input" value={toCity} onChange={e => setToCity(e.target.value)} placeholder="Destination" />
              </div>
              <div>
                <div className="fg-label">Vehicle Type</div>
                <select className="fg-input" value={vtype} onChange={e => setVtype(e.target.value)}>
          ...
          {[
            ...
            <div key={item.title} className="trust-item">
              <div className="trust-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="#0f6ec8" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
              </div>
      ...
      <div className="page-body">
        ...
        <div className="info-box">
          <p>Yatra Travel India offers <strong>tempo traveller on rent in Kanpur</strong> for local sightseeing, wedding guest transfers,
          ...
          Transparent pricing — fuel, tolls, and driver allowance all included. Call <strong>9044019511</strong> to book.</p>
        </div>
        ...
        {/* OPTIONS TABLE */}
        <h2 className="section-title">Tempo Traveller Options in Kanpur — Full Comparison</h2>
        ...
        <div className="fare-table-wrap">
          <table className="fare-table">
            <thead>
              <tr><th>Route</th><th>Distance</th><th>Vehicle</th><th>Booking</th></tr>
            </thead>
            <tbody>
        ...
        <p className="table-note">* Fares depend on vehicle size and trip duration. Call 9044019511 for an exact quote.</p>
        ...
        {/* ── INCLUSIONS / EXCLUSIONS (NO ICONS — clean list) ── */}
        ...
        <div className="ei-split">
          <div className="ei-box">
            <div className="ei-box-header">
              <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" width="20" height="20"><polyline points="20 6 9 17 4 12"/></svg>
              <span>&nbsp;INCLUDED</span>
            </div>
            <ul>
          ...
          <div className="ei-box">
            <div className="ei-box-header excl">
              <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" width="20" height="20"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              <span>&nbsp;EXCLUDED</span>
            </div>
            <ul>
              {EXCLUDED.map(item => <li key={item} style={{ color: "#374151" }}>{item}</li>)}
            </ul>
          </div>
        </div>
        ...
        <div className="routes-grid">
          {OUTSTATION_ROUTES.map(r => (
            <div key={r.title} className="route-card">
              <h4>{r.title}</h4>
              <div className="route-meta">{r.meta.map(m => <span key={m}>{m}</span>)}</div>
              <p>{r.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="cta-banner">
          ...
          <div className="cta-buttons">
            <button className="btn-cta-primary"   onClick={() => window.location.href = "tel:+919044019511"}>Call Now — +91 90440 19511</button>
            <button className="btn-cta-secondary" onClick={() => window.open("https://wa.me/919044019511")}>WhatsApp Us</button>
          </div>
        </div>
        ...
        <div className="popular-routes-section">
          <div className="network-section">
            <div className="network-title-row">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>
      ...
      <a href="https://wa.me/919044019511" className="wa-float" target="_blank" rel="noreferrer" aria-label="WhatsApp">
      ...
      <a href="tel:+919044019511" className="call-float" aria-label="Call Now">
      ...
      {/* TOAST */}
      <div id="toast-tt" className={toast.show ? "show" : ""}>{toast.msg}</div>
    </>
  );
}

/* ═══════════════════════════════════════════
   VEHICLE CARD — matches screenshot design
═══════════════════════════════════════════ */
function VehicleCard({ vehicle, onSelect }) {
  const isOrange = vehicle.badgeColor === "orange";
  const BusIcon = () => (
    <svg viewBox="0 0 80 40" fill="none" style={{ width: 72, height: 72, opacity: 0.35 }}>
  ...
  return (
    <div className="vehicle-card">
      <div className="vc-img">
        ...
        <span className={`vc-badge${isOrange ? "" : " blue"}`}>{vehicle.badge}</span>
      </div>
      <div className="vc-body">
        <h3>{vehicle.title}</h3>
        <div className="vc-specs">
          {vehicle.specs.map(s => (
            <div key={s.label} className="vc-spec-row">
              <span className="vc-spec-label">{s.label}</span>
              <span className="vc-spec-value">{s.value}</span>
            </div>
          ))}
        </div>
        <div className="vc-tags">
          {vehicle.tags.map(t => (
            <span key={t} className={`vc-tag${isOrange ? " orange" : ""}`}>{t}</span>
          ))}
        </div>
        <button
          className={`btn-book-card${vehicle.btnColor === "blue" ? " blue" : ""}`}
