        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          <header className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Discover the hidden gems of India through the stories of our explorers.
            </p>
          </header>
          ...
          {blogs.length === 0 ? (
            <div className="bg-white rounded-3xl p-20 text-center shadow-sm">
              ...
              <Link href="/admin/create" className="text-blue-600 hover:underline mt-4 inline-block">
                Go to Editor →
              </Link>
            </div>
          ) : (
            <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
              {blogs.map((blog) => (
                <article key={blog._id} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col">
                  <Link href={`/blogs/${blog.slug}`}>
                    <div className="aspect-[16/10] bg-gray-200 relative overflow-hidden">
                      {blog.coverImage ? (
                        <img 
                          ...
                          loading="lazy" // SEO: Best practice for list images
                        />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-gray-400 font-bold uppercase tracking-tighter text-4xl opacity-20">
                          Yatra
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-8 flex flex-col flex-grow">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <time dateTime={blog.createdAt} className="text-gray-400 text-xs">
